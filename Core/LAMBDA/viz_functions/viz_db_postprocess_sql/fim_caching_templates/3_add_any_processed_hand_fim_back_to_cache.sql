...
INSERT INTO handfim_cache.hydrotable_cached_max(hand_id, model_version, max_rc_discharge_cfs, max_rc_stage_ft)
SELECT DISTINCT
    fim.hand_id,
    fim.model_version,
...
LEFT OUTER JOIN handfim_cache.hydrotable_cached_max AS hcm ON fim.hand_id = hcm.hand_id
WHERE fim.prc_method = 'HAND_Processing' 
...
INSERT INTO handfim_cache.hydrotable_cached_geo (hand_id, rc_stage_ft, geom)
SELECT
    fim_geo.hand_id,
    fim_geo.rc_stage_ft,
    fim_geo.geom
...
LEFT OUTER JOIN handfim_cache.hydrotable_cached_geo AS hcg ON fim_geo.hand_id = hcg.hand_id AND fim_geo.rc_stage_ft = hcg.rc_stage_ft
...
INSERT INTO handfim_cache.hydrotable_cached_zero_stage (hand_id, rc_discharge_cms, note)
SELECT
    fim.hand_id,
    fim.rc_discharge_cms,
    fim.note
FROM {db_fim_table}_zero_stage AS fim
LEFT OUTER JOIN handfim_cache.hydrotable_cached_zero_stage AS hczs ON fim.hand_id = hczs.hand_id
